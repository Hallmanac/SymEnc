...
namespace SymEnc.Core
{
  public class SymmetricEncyrptionSvc : ISymmetricEncyrptionSvc
  {
    ...
    ///     For use cases that require interaction outside of the site, we would generate a new random (private) key for each
    ///     instance where we would store the generated key as well as the client app would store the generated private key.
    ///   </para>
    /// </summary>
    public string Default256BitKey { get; private set; }
    ...
    ///   generated
    ///   for each encryption process (i.e. different every time the encryption is run). This will happen automatically in our
    ///   Decrypt
    ...
    /// <summary>
    ///   Decrypts a given cipher text based on the provided key material. The initialization vector should be prefixed to the
